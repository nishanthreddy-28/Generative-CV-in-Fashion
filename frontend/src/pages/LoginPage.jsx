...
export default function LoginPage() {
  ...
  const handleError = (error, provider) => {
    if (error.code === 'auth/api-key-not-valid' || error.message.includes('API_KEY')) {
      ...
      toast.error(`Failed to log in with ${provider}: ${error.message}`);
    }
  };
  ...
  const handleSocialSignIn = async (providerName) => {
    if (providerName === 'Google') {
      ...
      toast.error(`${providerName} login is not configured yet.`);
    }
  };
    ...
    <div className="flex min-h-screen bg-background">
      ...
      <div className="flex w-full flex-col justify-center px-4 sm:px-6 lg:w-1/2 lg:px-20 xl:px-32">
        ...
        >
          ...
          <div>
            <h2 className="text-3xl font-semibold tracking-tight text-foreground">Log in to your account</h2>
          ...
          >
            <blockquote className="space-y-6">
              <p className="text-3xl font-medium leading-relaxed text-white">
                "Drape & Drop has completely transformed how I organize my wardrobe. The AI styling suggestions are incredibly accurate to my personal taste."
              </p>
              <footer className="flex items-center gap-4">
                <div className="h-12 w-12 rounded-full border-2 border-white/20 overflow-hidden">
                  <img src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=250&auto=format&fit=crop" alt="User avatar" className="h-full w-full object-cover" />
                </div>
                <div>
                  <div className="text-base font-semibold text-white">Elena Rodriguez</div>
                  <div className="text-sm text-zinc-400">Fashion Content Creator</div>
                </div>
              </footer>
            </blockquote>
          </motion.div>
